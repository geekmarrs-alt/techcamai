...
def test_ingest_stale_camera_id_does_not_fallback_to_matching_url(client):
    """A stale explicit worker camera id must not create alerts on a different camera."""
    cam_r = client.post("/cameras", json={"name": "FallbackCam", "ip": "192.168.2.50", "username": "u", "password": "p"})
    cam_id = cam_r.json()["id"]
    client.post("/rules", json={"name": "Fallback motion", "camera_id": cam_id, "min_conf": 0.1, "cooldown_sec": 0})

    r = client.post("/ingest/detection", json={
        "camera_snapshot_url": "http://192.168.2.50/snapshot",
        "camera_id": 999999,
        "label": "motion",
        "conf": 0.9,
    }, headers=WORKER_HEADERS)
